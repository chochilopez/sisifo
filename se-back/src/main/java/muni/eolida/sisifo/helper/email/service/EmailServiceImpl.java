...
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    public EntidadMensaje<EmailModel> enviarEmailSimple(EmailCreation emailCreation) {
        try {
            ...
            log.error(mensaje);
            return new EntidadMensaje<EmailModel>(null, null, mensaje, 204);
        }
    }

    @Override
    public EntidadMensaje<EmailModel> buscarPorId(Long id) {
        ...
        if (objeto.isEmpty()) {
            ...
            log.warn(mensaje);
            return new EntidadMensaje<EmailModel>(null, null, mensaje, 202);
        } else {
            String mensaje = "Se encontro una entidad Email.";
            log.info(mensaje);
            return new EntidadMensaje<EmailModel>(objeto.get(), null, mensaje, 200);
        }
    }

    @Override
    public EntidadMensaje<EmailModel> buscarTodas() {
        ...
        if (listado.isEmpty()) {
            ...
            log.info(mensaje);
            return new EntidadMensaje<EmailModel>(null, listado, mensaje, 200);
        }
    }
    ...
    @Override
    public EntidadMensaje<EmailModel> insertar(EmailCreation model) {
        try {
            ...
            log.info(mensaje);
            return new EntidadMensaje<EmailModel>(objeto, null, mensaje, 201);
        } catch (Exception e) {
    ...
    @Override
    public EntidadMensaje<EmailModel> actualizar(EmailModel model) {
        try {
            log.info("Actualizando la entidad Email: {}.",  model);
            if (model.getId() != null) {
                EntidadMensaje<EmailModel> entidad = this.buscarPorId(model.getId());
    ...
    @Override
    public EntidadMensaje<EmailModel> destruir(Long id) {
        try {
            log.info("Destruyendo la entidad Email con id: {}.", id);
            EntidadMensaje<EmailModel> objeto = this.buscarPorId(id);
